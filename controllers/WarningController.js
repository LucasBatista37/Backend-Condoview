...
const createNotice = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ errors: ["Houve um erro ao criar o aviso.", error.message] });
  }
};
  ...
  try {
    ...
    if (!notice) {
      return res
        .status(404)
        .json({
          errors: ["Aviso não encontrado ou não pertence ao seu condomínio."],
        });
    }
    ...
    res
      .status(422)
      .json({ errors: ["Erro ao atualizar o aviso.", error.message] });
  }
};
...
const deleteNotice = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ errors: ["Erro ao deletar o aviso.", error.message] });
  }
};
